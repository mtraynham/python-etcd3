...
class WatchThread(threading.Thread):
    ...
    def run(self) -> None:
        request_iterator = (
            ...
            for request in self.request_iterator()
        )
...
class Watch:
    ...
    def create_iterator(
        ...
        def callback(
            response: typing.Optional[rpc_pb2.WatchResponse]
        ) -> None:
        ...
        watch_create = self.create_callback(
            request=request,
            callback=callback,
            error_callback=error_callback
        )
        ...
        def cancel() -> None:
            self.cancel(watch_create.watch_id)
    ...
    def create_once(
        ...
        watch_response, cancel = self.create_iterator(
            request=request,
            timeout=timeout
        )
        try:
