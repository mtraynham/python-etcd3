...
class ExtendedClient:
    def __init__(
        ...
        watch_timeout: typing.Optional[float] = None
    ):
        self._client = client
    ...
    def watch_response(
        ...
        **kwargs: typing.Any
        ...
        def callback(response: typing.Optional[rpc_pb2.WatchResponse]) -> None:
            response_queue.put(response)

        watch_create = self._watcher.create(
            request=request,
            callback=callback
        )

        def cancel() -> None:
            self._watcher.cancel(watch_create.watch_id)
    ...
    def watch_once_response(
        ...
        **kwargs: typing.Any
        ...
        watch_response, cancel = self.watch_response(
            key=key,
            **kwargs
        )
        try:
            return next(watch_response)
        finally:
            cancel()
